...
class ResUnet_ASPP_CBAM:
    ...
    # def ResNetBlock(self, inputTensor, numFilters, kernelSize=3):
    #     x = Conv2D(filters=64, kernel_size=(1, 1), strides=1, padding='same', use_bias=False)(inputTensor)
    #     divide = int(x.shape[3] / 4)
    #     x1 = x[:, :, :, 0:divide]
    #     x2 = x[:, :, :, divide:2 * divide]
    #     x3 = x[:, :, :, 2 * divide:3 * divide]
    #     x4 = x[:, :, :, 3 * divide:]
    #
    #     x2_1 = tf.keras.layers.Conv2D(filters=divide, kernel_size=(kernelSize, kernelSize),
    #                                kernel_initializer='he_normal', padding='same')(x2)
    #     x3_1 = tf.keras.layers.Conv2D(filters=divide, kernel_size=(kernelSize, kernelSize),
    #                                kernel_initializer='he_normal', padding='same')(x3 + x2_1)
    #     x4_1 = tf.keras.layers.Conv2D(filters=divide, kernel_size=(kernelSize, kernelSize),
    #                                kernel_initializer='he_normal', padding='same')(x4 + x3_1)
    #     y = x1 + x2_1 + x3_1 + x4_1
    #
    #     y = tf.keras.layers.BatchNormalization()(y)
    #     y = tf.keras.layers.Activation('relu')(y)
    #     y = Conv2D(filters=numFilters, kernel_size=(1, 1), strides=1, padding='same', use_bias=False)(y)
    #     y = self.Channal(y, numFilters)
    #
    #     return y
